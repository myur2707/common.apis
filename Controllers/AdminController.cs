...
namespace WebApiCampusConcierge.Controllers
{
    public class AdminController : ApiController
    {
        ...
        //admin new Start
        [HttpPost]
        public string getNewUsers_admin([FromBody]JObject parameter)
        {
            ...
            if (security_flag == true)
            {
                ...
                dtstudent_rate = objmasters.totaluser();
        ...
        #region Quintana service
        public string custome_alert_QUA([FromBody]JObject parameter)
        {
            BLReturnObject objBLReturnObject = new BLReturnObject();
            string uni = "";
            string student_achive = "";
            ...
            try
            {
                if (parameter["university"].ToString() != "" && parameter["university"].ToString() != null)
                {

                    uni = parameter["university"].ToString();
                }

                if (parameter["message"].ToString() == "" && parameter["message"].ToString() == null)
                {
                    return "Message Not Empty";
                }

                #region alert message
                DataTable dt_student = objmasters.GetUserAlert(uni);
                if (dt_student != null && dt_student.Rows.Count > 0)
                {
                    ...
                    objBLReturnObject = objmasters.alert_customeQUA(obj_DSAdmin);
                    if (objBLReturnObject.ExecutionStatus == 1)
                    {
                        return "Pass";
                        //result = "Pass";
        ...
        #region New Admin
        [HttpGet]
        public string GetUserMaster(string app)
        {
            ...
            try
            {
                dtSessionDetails = objmasters.getusermaster_admin("");
